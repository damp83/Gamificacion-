...
async function descargarInforme(clave) {
  ...
  toast(guardado && guardado.ok === false
    ...
    : `Informe de ${st.profile.explorer_name} descargado ✓ Ábrelo y pulsa «Guardar como PDF».`,
    5200);
}
...
function pintarBotonDeInformes(d) {
  ...
  nota.textContent = 'Un archivo con un informe por página. Se abre, se pulsa «Guardar como PDF» '
    + 'y sale el PDF de toda la clase. Para escribirle una nota a una familia concreta, usa el '
    + 'botón de su ficha.';
  caja.appendChild(nota);
}
...
async function descargarInformesDeClase() {
  ...
  if (!(await askConfirm(`Se van a preparar ${fichas.length} informes, uno por página, en un solo
    archivo. Al abrirlo, el botón de arriba lo guarda como PDF. Se usa la nota que ya tengas
    escrita de cada familia; las que falten salen sin ella.`,
  ...
  toast(guardado && guardado.ok === false
    ? 'No se ha podido descargar. Prueba desde Configuración → Copia de seguridad.'
    : `${cuerpos.length} informe(s) listos ✓ Ábrelos y pulsa «Guardar como PDF».${fallos.length
