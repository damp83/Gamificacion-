    ...
    <div class="student-card${s.needsHelp ? ' student-alert' : ''}">
      ...
      ${(s.tieneDiario || (s.id && cloudEnabled() && cloudUser())) ? `<div class="student-acciones">
        ...
        <button class="btn btn-secondary btn-small student-informe"
          data-informe="${esc(s.clave || s.id)}">${ico('logbook')} Informe en PDF</button>
...
${antiguas.length ? `<details class="antes"><summary>Lo que se dijo antes</summary>
  ...
  /* ── Lo que pide un informe de centro ──
     «Lo que ya le sale» y «en lo que está trabajando» son los dos extremos, y
     están bien para una familia. Un informe oficial pide además las dos cosas
     que nombra la ley: los SABERES recorridos en el periodo y los CRITERIOS
     con su calificación. Van en su propia sección y al final, después de lo
     que se lee en casa, porque el orden de esta hoja no cambia: primero lo
     que ha escrito una persona mirando a ese niño. */
  /* ── Y de qué periodo hablan estas dos tablas ──
     Del trimestre, cuando el diario tiene el desglose. Cuando no lo tiene —un
     diario anterior a que existieran esos contadores— NO se enseñan tablas a
     cero: se cuenta el curso entero y se dice que es el curso entero. Una
     columna vacía en la hoja que se archiva no dice «faltan datos», dice «este
     niño no ha hecho nada», que es exactamente lo contrario de la verdad. */
  const criterios = ATLAS_CONFIG.criterios || [];
  let periodoOficial = tri ? iTri : null;
  if (tri && criterios.length) {
    const prueba = evidenciaDeCriterios(s, criterios, iTri);
    if (!prueba.hayTrimestres) periodoOficial = null;
  } else if (tri) {
    /* Sin criterios declarados se mira igual, que los saberes también se
       acotan y también pueden salir a cero por lo mismo. */
    const m = (s.metrics && s.metrics.errors_by_concept) || {};
    if (!Object.keys(m).some(id => m[id] && m[id].tri)) periodoOficial = null;
  }
  const triOficial = periodoOficial === null ? null : tri;
  const saberes = saberesTrabajados(s, periodoOficial);
  const evidencia = criterios.length
    ? evidenciaDeCriterios(s, criterios, periodoOficial) : null;

  return cuerpoDelInforme(s, o, {
    dominados, flojos, pozos, camaras, superadasTri, log, minutos, sesiones, cubo, tri, hoy,
    lista, bloqueNota, enEspanol, vacio, saberes, criterios, evidencia, triOficial
  });
}
...
const ESTILO_INFORME = `<style>
  ...
  /* La parte oficial: misma hoja, otra mitad. La raya y el gris de fondo
     dicen sin palabras que lo de abajo se lee de otra manera. */
  .oficial { margin-top: 30px; }
  .corte { border: none; border-top: 2px dashed #cbb99a; margin: 0 0 16px; }
  .oficial-intro { color: #6b5d4a; font-size: .88rem; margin: 0 0 18px; }
  .sub-seccion { color: #6b5d4a; font-size: .88rem; margin: 2px 0 10px; }
  .area h3 { font-size: .95rem; margin: 16px 0 4px; color: #8a5a0b; }
  table { width: 100%; border-collapse: collapse; font-size: .87rem; margin: 4px 0 10px; }
  th { text-align: left; font-size: .78rem; text-transform: uppercase;
       letter-spacing: .04em; color: #6b5d4a; border-bottom: 1px solid #e0d3ba;
       padding: 4px 8px 4px 0; }
  td { padding: 5px 8px 5px 0; border-bottom: 1px solid #f0e7d6; vertical-align: top; }
  td.num, th.num { text-align: right; font-variant-numeric: tabular-nums; white-space: nowrap; }
  /* Las tablas de saberes van una por área, y cada una se medía sola: las
     columnas bailaban de un área a otra y no se podían comparar de un vistazo.
     Con el ancho fijo, todas caen en la misma vertical. */
  .tabla-saberes { table-layout: fixed; }
  .tabla-saberes th:nth-child(1) { width: 46%; }
  .tabla-saberes th:nth-child(2) { width: 13%; }
  .tabla-saberes th:nth-child(3) { width: 15%; }
  .tabla-criterios { table-layout: fixed; }
  .tabla-criterios th:nth-child(1) { width: 10%; }
  .tabla-criterios th:nth-child(3) { width: 13%; }
  .tabla-criterios th:nth-child(4) { width: 13%; }
  .tabla-criterios th:nth-child(5) { width: 21%; }
  td.codigo { font-weight: 700; white-space: nowrap; }
  .pocos { color: #6b5d4a; font-style: italic; }
  /* La barra del PDF: lo primero que se ve al abrir el informe, porque es lo
     que se ha venido a hacer con él. En papel desaparece. */
  .barra-pdf { display: flex; align-items: center; gap: 12px; flex-wrap: wrap;
               background: #f6efe2; border: 1px solid #e0d3ba; border-radius: 10px;
               padding: 12px 14px; margin: 0 0 26px; font-size: .85rem; color: #6b5d4a; }
  .barra-pdf button { font: inherit; font-weight: 700; font-size: .9rem; cursor: pointer;
                      background: #37622f; color: #f3f9ef; border: 0;
                      border-radius: 999px; padding: 9px 18px; }
  .barra-pdf button:hover { background: #2b4d25; }
  .aviso-criterios { color: #6b5d4a; font-size: .82rem; margin: 6px 0 0; }
  /* Una tabla partida entre dos hojas se lee fatal: se evita cuando cabe. */
  @media print {
  ...
  /* ── Para el PDF ──
     El PDF lo hace el propio navegador al imprimir, así que el margen de la
     hoja se declara aquí y no lo decide la impresora de cada casa. A4 con dos
     centímetros por todos los lados es lo que espera una secretaría. */
  @page { size: A4; margin: 18mm 16mm; }
  @media print {
    body { margin: 0; max-width: none; font-size: 11.5pt; }
    ...
    /* La parte oficial empieza en su propia hoja: es la que se archiva, y
       partida a mitad de página se fotocopia mal. */
    .oficial { break-before: page; page-break-before: always; }
    /* Y el botón de imprimir no se imprime, claro. */
    .barra-pdf { display: none !important; }
  }
</style>
`;
...
/* ── El PDF ──
   ...
   Sin librería, y no por ahorrar: una librería de PDF se descarga de un CDN, y
   esta aplicación tiene que funcionar en un aula sin wifi y dentro del archivo
   suelto que se abre con doble clic. Lo que sí hay en todos los dispositivos
   —el iPad del aula incluido— es la impresión del navegador, y su «Guardar
   como PDF» produce un PDF de verdad, con el texto seleccionable, sin
   descargar nada.

   Así que el informe se abre con su propio botón arriba. Un toque y sale el
   diálogo de imprimir; ahí se elige guardar en PDF. En el iPad es «Compartir →
   Guardar en Archivos». El botón no se imprime, y el margen de la hoja lo pone
   el propio informe para que no dependa de cómo tenga cada uno la impresora. */
const BARRA_PDF = `<div class="barra-pdf">
  <button type="button" id="pdf" autofocus>Guardar como PDF o imprimir</button>
  <span>Se abre el diálogo de impresión. Elige «Guardar como PDF» como destino.</span>
</div>`;

const GUION_PDF = `<script>
...
function cuerpoDelInforme(s, o, d) {
  const { dominados, flojos, pozos, camaras, superadasTri, log, minutos, sesiones,
          cubo, tri, hoy, lista, bloqueNota, enEspanol, vacio,
          saberes, criterios, evidencia, triOficial } = d;
  const dentro = f => !tri || !f || (String(f) >= tri.start && String(f) <= tri.end);
...
/* ── La parte oficial, y por qué va aparte ──

   Esta hoja tenía una regla, y era buena: sin notas, sin porcentajes, sin
   comparaciones. Está escrita para que una familia entienda por dónde va su
   hijo sin traducir nada, y un «68 %» en medio de esa lectura no informa, solo
   asusta.

   Pero un informe de evaluación tiene que acreditar dos cosas que la ley
   nombra: los saberes trabajados y los criterios con su calificación. Así que
   la hoja pasa a tener dos mitades, y la raya que las separa es de verdad: lo
   de arriba se lee, lo de abajo se acredita. Arriba sigue sin haber ni un
   número de esos; abajo están todos, con los intentos al lado, que es la única
   forma de que un nivel signifique algo.
...
<hr class="corte">
<p class="oficial-intro">A partir de aquí va la parte que el centro necesita para el expediente:
qué se ha trabajado y cómo va cada criterio de evaluación. Son los mismos datos de arriba,
contados en el lenguaje de la programación.</p>
${aviso}
${s1}
${s2}
</div>`;
...
/* ── Los saberes recorridos ──
   La lista completa de lo trabajado en el periodo, con su área y con cuántas
   veces se ha practicado cada cosa. No es una selección como las dos secciones
   de arriba: es el temario que de verdad se ha tocado, que es lo que un
   informe de centro tiene que poder acreditar.

   Los intentos van siempre al lado del nivel, y lo que se ha practicado poco
   sale sin nivel y con sus intentos a la vista. Un 100 % de tres respuestas no
   es un sobresaliente: es un 100 % de tres respuestas, y esta hoja no puede
   decir lo contrario. */
function bloqueSaberes(saberes, tri) {
  if (!saberes || !saberes.length) return '';
  return `<h2>Saberes trabajados${tri ? ` en el ${esc(tri.name)}` : ''}</h2>
<p class="sub-seccion">Lo que se ha practicado en la aplicación durante el periodo, con las veces
que se ha trabajado cada cosa.</p>
${saberes.map(g => `<div class="area">
  <h3>${esc(g.area)}</h3>
  <table class="tabla-saberes">
    <thead><tr><th>Saber</th><th class="num">Veces</th><th class="num">Acierto</th><th>Nivel</th></tr></thead>
    <tbody>${g.saberes.map(x => `<tr>
      <td>${esc(x.label)}</td>
      <td class="num">${x.intentos}</td>
      <td class="num">${x.pct} %</td>
      <td>${x.nivel ? esc(x.nivel) : '<span class="pocos">pocas veces todavía</span>'}</td>
    </tr>`).join('')}</tbody>
  </table>
</div>`).join('')}`;
}
...
/* ── Los criterios calificados ──
   El puente entre lo que mide la aplicación y lo que pide el centro. Cada
   criterio es el que el docente tenga en su programación, con su código y su
   texto tal cual, y al lado la evidencia reunida: cuántos intentos, con qué
   acierto y qué nivel sale de ahí.

   Un criterio sin intentos suficientes no se califica. Podría rellenarse con
   el número que salga y quedaría una tabla completa, que es justo la tentación
   que hay que resistir: una tabla completa con dos datos detrás no es una
   evaluación, es una tabla. Se dice que falta evidencia y ya está. */
function bloqueCriterios(criterios, evidencia, tri) {
  if (!criterios || !criterios.length || !evidencia) return '';
  const porId = {};
  for (const f of evidencia.filas) porId[f.id] = f;
  const conNota = evidencia.filas.filter(f => f.nivel).length;
  return `<h2>Criterios de evaluación${tri ? ` · ${esc(tri.name)}` : ''}</h2>
<p class="sub-seccion">${conNota
    ? `Calificados <strong>${conNota} de ${criterios.length}</strong> con la evidencia recogida en la
       aplicación. El resto necesita más práctica antes de poder decir cómo va.`
    : 'Todavía no hay práctica suficiente para calificar ninguno.'}</p>
<table class="tabla-criterios">
  <thead><tr><th>Código</th><th>Criterio</th><th class="num">Intentos</th>
    <th class="num">Acierto</th><th>Calificación</th></tr></thead>
  <tbody>${criterios.map(c => {
    const f = porId[c.id] || { intentos: 0, pct: null, nivel: '' };
    return `<tr>
      <td class="codigo">${esc(c.codigo || '—')}</td>
      <td>${esc(c.texto || '')}</td>
      <td class="num">${f.intentos}</td>
      <td class="num">${f.pct === null ? '—' : f.pct + ' %'}</td>
      <td>${f.nivel ? `<strong>${esc(f.nivel)}</strong>`
        : '<span class="pocos">sin evidencia suficiente</span>'}</td>
    </tr>`;
  }).join('')}</tbody>
</table>
<p class="aviso-criterios">La calificación que sale aquí es la evidencia que ha recogido la
aplicación, no la nota del trimestre: esa la pone el maestro con todo lo demás que ve en clase.
El porcentaje y los intentos van siempre al lado para que se sepa de dónde sale cada nivel.</p>`;
}

/* ── Imprimir el informe, que es de donde sale el PDF ──
   ...
   PDF», y en un iPad el archivo se va a Archivos y abrirlo desde ahí es un
   viaje. Ahora el botón lleva directo al diálogo de impresión, que es donde
   está «Guardar como PDF».

   Va en un MARCO APARTE y no en la propia página por el CSS: el informe trae
   sus estilos escritos con selectores desnudos —`body`, `h1`, `table`— y
   metidos en la app se la llevarían por delante. Un iframe los deja donde
   tienen que estar, que es para lo que existen.

   Y el marco se coloca fuera de la pantalla a su tamaño real, no a cero ni
   con opacidad cero: hay navegadores que imprimen en blanco un marco de un
   píxel, y eso no se ve hasta que alguien manda el PDF a una familia. */
function imprimirInforme(html) {
  return new Promise(resolve => {
    const viejo = document.getElementById('marco-impresion');
    ...
    const terminar = ok => {
      if (acabado) return;
      acabado = true;
      /* El marco se queda un rato: quitarlo en cuanto vuelve `print()` corta
         el diálogo en los navegadores que no bloquean mientras está abierto. */
      setTimeout(() => { const m = document.getElementById('marco-impresion'); if (m) m.remove(); }, 60000);
      resolve(ok);
    };
    ...
    /* Si el marco no llega a cargar —nada debería impedirlo, pero el día que
       pase no puede quedarse el docente mirando una pantalla que no hace
       nada— se cae al archivo, que es lo que había antes. */
    setTimeout(() => terminar(false), 8000);
  });
}
...
async function descargarInforme(clave) {
  ...
  if (await imprimirInforme(html)) {
    toast('Elige «Guardar como PDF» como destino. En iPad: Compartir → Guardar en Archivos.', 5200);
    return;
  }
  /* Si la impresión no sale, queda el archivo, que es lo que había antes. */
  const guardado = await guardarArchivo(informeFileName(st), html, 'text/html');
  toast(guardado && guardado.ok === false
    ? 'No se ha podido ni imprimir ni descargar. Prueba desde otro navegador.'
    : `No se ha podido abrir la impresión: se ha descargado el informe de ${
        st.profile.explorer_name}. Ábrelo y pulsa «Guardar como PDF».`,
    5600);
}
...
function pintarBotonDeInformes(d) {
  ...
  nota.textContent = 'Un informe por página, todos en el mismo PDF. Se abre el diálogo de '
    + 'impresión y se elige «Guardar como PDF». Para escribirle una nota a una familia concreta, '
    + 'usa el botón de su ficha.';
  caja.appendChild(nota);
}
...
async function descargarInformesDeClase() {
  ...
  if (!(await askConfirm(`Se van a preparar ${fichas.length} informes, uno por página, y se abrirá
    el diálogo de impresión para guardarlos como PDF. Se usa la nota que ya tengas escrita de cada
    familia; las que falten salen sin ella.`,
    `Preparar ${fichas.length} informes`))) return;
  ...
  if (await imprimirInforme(html)) {
    toast(`${cuerpos.length} informe(s) listos, uno por página. Elige «Guardar como PDF».${
      fallos.length ? ` No se ha podido con: ${fallos.slice(0, 3).join(', ')}${
        fallos.length > 3 ? '…' : ''}` : ''}`, fallos.length ? 6000 : 5200);
    return;
  }
  const guardado = await guardarArchivo(nombre, html, 'text/html');
  toast(guardado && guardado.ok === false
    ? 'No se ha podido ni imprimir ni descargar. Prueba desde otro navegador.'
    : `No se ha podido abrir la impresión: se han descargado los ${cuerpos.length} informes.${fallos.length
